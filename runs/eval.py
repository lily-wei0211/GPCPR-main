...
def evaluate_metric(logger, pred_labels_list, gt_labels_list, label2class_list, test_classes):
    ...
    :return: iou: scaler
    # pred_labels_list, gt_labels_list, label2class_list, test_classes
    # query_pred, query_label, sampled_classes, test_classes
    """
...
def test_few_shot(test_loader, learner, logger, test_classes):
    ...
    for batch_idx, (data, sampled_classes) in enumerate(test_loader):
        ...
        if (batch_idx + 1) % 100 == 0:
            ...
            logger.cprint('[Eval] Iter: %d | Loss: %.4f | Acc: %.2f | %s' % (batch_idx + 1, loss.detach().item(), accuracy, str(datetime.now())))
